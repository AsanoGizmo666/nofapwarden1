...
def show_status(m):
    ...
    test_bar = get_progress_bar(metrics['testosterone'])
    dopa_bar = get_progress_bar(metrics['dopamine'])
    telo_bar = get_progress_bar(metrics['telomeres'] * 10)
    energy_bar = get_progress_bar(metrics['energy'])
    total_progress = (metrics['testosterone'] + metrics['dopamine'] + metrics['energy']) / 3
    total_bar = get_progress_bar(total_progress)
...
def show_level(m):
    ...
    if current_level == 0:
        level_info = {"name": "ПОДГОТОВИТЕЛЬНАЯ ФАЗА", "effect": "Детоксикация и адаптация", "scientific": "Базовая гормональная перестройка"}
        next_level = LEVELS[1]
        ...
        if current_level < 4:
            ...
            next_level = {"name": "МАКСИМАЛЬНЫЙ УРОВЕНЬ", "days": 365, "effect": "Все системы оптимизированы"}
            days_left = 0
    
    # Создаем прогресс-бар
    progress_percent = round(days/next_level['days']*100 if current_level < 4 else 100, 1)
...
def group_lab(m):
    ...
    test_bar = get_progress_bar(group_testosterone)
    energy_bar = get_progress_bar(group_energy)
    
    # Анализ эффективности
    if avg_days > 14:
        synergy = "Высокая синергия (BDNF +45%)"
        readiness = "Готовы к уровню 3" if avg_days > 21 else ""
    elif avg_days > 7:
        synergy = "Умеренная синергия"
        readiness = "Готовы к уровню 2"
    else:
        synergy = "Формирование группы"
        readiness = ""
    
    # Определяем статус максимального стрика
    max_status = "КВАНТОВОЕ СОЗНАНИЕ" if max_days > 90 else "ОПТИМИЗАЦИЯ"
    min_status = "ДЕТОКС" if min_days < 7 else "АДАПТАЦИЯ"
...
• Максимальный: {max_days} дней ({max_status})
• Минимальный: {min_days} дней ({min_status})

📡 *КОЛЛЕКТИВНЫЕ ПОКАЗАТЕЛИ:*
🏋️ Групповой тестостерон: {test_bar} {round(group_testosterone)}%
⚡ Совокупная энергия: {energy_bar} {round(group_energy)}%
...
👤 Имя: {stats['name']}
🔥 Текущий стрик: {stats['days']} дн.
📉 Всего срывов: {stats['attempts']}
🏆 Статус: {stats['status']}

🔬 Детальные метрики: `/статус`
💡 Рекомендации: `/анализ`"""
